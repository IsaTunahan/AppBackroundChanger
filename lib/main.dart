import 'dart:math';
	...
	void main() => runApp(MyApp()); //1- main fonksiyonu uygulamayı başlatır ve MyApp sınıfını çağırır.
	 ...
	 class MyApp extends StatefulWidget { //2- MyApp sınıfı, StatefulWidget sınıfından türetilir ve _MyAppState sınıfını çağırır. 
	...
	void _changeColor(){//4- _changeColor fonksiyonu, yeni bir rastgele renk oluşturmak için Random() sınıfını kullanır. 
	...
	@override
	Widget build(BuildContext context){//5- Build fonksiyonu, arayüzü oluşturmak için kullanılır. 
	                                  //Bu fonksiyon, MaterialApp widget'ı ile başlar ve Scaffold widget'ıyla devam eder.
	

	return MaterialApp(
	  home: Scaffold(//6- Scaffold widget'ı, uygulama çerçevesini oluşturur ve bir AppBar widget'ı ile birlikte görüntülenir. 
	                //AppBar widget'ı, uygulama başlığını içerir.
	    appBar: AppBar(title: const Text('Backround Color Changer'),
	    ),
	    body: Container(//7-  Container widget'ı, uygulama arka plan rengini temsil eder ve _backroundColor değişkenine bağlanır.
	      ...
	      child: Center(//8 -Center widget'ı, butonu merkezlemek için kullanılır ve bir ElevatedButton widget'ı içerir.
	        ...
	        child: ElevatedButton(
	          ...
	          style: ElevatedButton.styleFrom(//9- ElevatedButton widget'ı, kullanıcının tıklamasını yakalamak için bir 
	            ...
	            textStyle: TextStyle(fontSize: 50,backgroundColor: _textColor)
	          ),
	          )
	          ),
